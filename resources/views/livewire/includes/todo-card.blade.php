    ...
    <div class="flex justify-between space-x-2">
        <div class="flex">
            @if ($todo->is_completed)
                <input wire:click='togglechecked({{ $todo->id }})' class="me-2" type="checkbox" checked>
            @else
                <input wire:click='togglechecked({{ $todo->id }})' class="me-2" type="checkbox">
            @endif

            @if ($todoId === $todo->id)
                <div class="">
                    <input wire:model="editName" type="text" id="name" placeholder="Todo.."
